  ...
  return (
    <header className="flex w-full flex-wrap justify-between items-center px-4 py-2">
      <Link href="/" className="flex items-center space-x-2 text-blue-500">
        <Image src="/trendify.png" alt="trendify-logo" width={62} height={62} />
        <span className="family-lobster text-3xl hidden md:inline">
          Trendify
        </span>
      </Link>

      <div className="flex items-center space-x-2 mt-4 sm:mt-2 flex-1 justify-end">
        <Form
          action="/search"
          className="hidden lg:flex bg-gray-100 text-gray-800 rounded-full border"
        >
          <div className="relative group">
            ...
            <input
              type="text"
              name="query"
              className="peer font-semibold group-hover:bg-gray-200 hover:bg-gray-200 px-13 py-2 rounded-full border w-full max-w-4xl"
        ...
        <Link
          href="/cart"
          className="relative flex justify-center sm:justify-start sm:flex-none items-center -space-x-1 font-bold py-2 px-2 pr-1 pl-2.5 lg:px-2.5 rounded"
        >
          <TrolleyIcon className="w-10 h-10" />
          <span className="absolute top-1 -right-2 lg:top-1 lg:right-4 bg-black text-white rounded-full w-6 h-4.5 flex items-center justify-center text-xs">
            {itemCount}
          </span>
